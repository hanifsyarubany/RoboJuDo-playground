...
class MujocoEnv(Environment):
    ...
    def reset_to_pose(self, root_pos, root_quat_xyzw, dof_pos):
        """Sim-only: place the robot at an explicit full-body pose.

        Mirrors ProtoMotions' ``test_tracker_mujoco.set_initial_pose``: sets the
        free-joint position/orientation AND every actuated joint, then refreshes
        derived state.  Used to snap the robot into the motion's frame-0 pose so
        the sim starts already matching the reference (correct root height,
        orientation and joint angles) instead of toppling from a neutral spawn.

        Args:
            root_pos: ``[3]`` world position of the root body (incl. height).
            root_quat_xyzw: ``[4]`` root orientation, ``xyzw`` order.
            dof_pos: ``[num_dofs]`` joint angles in this env's joint order.
        """
        self.data.qpos[0:3] = np.asarray(root_pos, dtype=np.float64)
        # MuJoCo qpos uses wxyz quaternion; incoming is xyzw.
        self.data.qpos[3:7] = np.asarray(root_quat_xyzw, dtype=np.float64)[[3, 0, 1, 2]]
        self.data.qpos[7:] = np.asarray(dof_pos, dtype=np.float64)
        self.data.qvel[:] = 0.0
        self.data.ctrl[:] = 0.0
        mujoco.mj_forward(self.model, self.data)  # pyright: ignore[reportAttributeAccessIssue]
        self.update()
