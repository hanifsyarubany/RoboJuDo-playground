...
class RlPipeline(Pipeline):
    ...
    def _teleport_to_motion_frame0(self) -> bool:
        """Sim-only: physically place the robot at motion frame 0.

        Mirrors ProtoMotions' ``test_tracker_mujoco.set_initial_pose`` so the
        RoboJuDo sim starts in the exact motion pose (root height, orientation
        and all joints) instead of relying on the hold-neutral -> transition
        flow.  Returns ``True`` if the teleport happened.

        No-op on real hardware: only the MuJoCo env exposes ``reset_to_pose``,
        and a real robot cannot be teleported.
        """
        if not hasattr(self.env, "reset_to_pose"):
            return False
        inner = self._inner_policy()
        player = getattr(inner, "_player", None)
        if player is None:
            return False
        frame0 = player.get_state_at_frame(0)
        root_pos = frame0["body_pos"][0]        # [3] world pos incl. height
        root_quat_xyzw = frame0["body_rot"][0]  # [4] xyzw
        init_dof = self.policy.get_init_dof_pos()  # frame-0 joints, env order
        self.env.reset_to_pose(root_pos, root_quat_xyzw, init_dof)  # pyright: ignore[reportAttributeAccessIssue]
        # Recompute heading offset from the now-matched pose (~ identity).
        self.policy.reset_alignment()
        logger.info(
            f"[sim] teleported robot to motion frame 0 "
            f"(root_h={float(root_pos[2]):.3f})"
        )
        return True
            ...
            match command:
                ...
                case "[SIM_REBORN]":
                    if hasattr(self.env, "reborn"):
                        ...
                        # Prefer snapping to motion frame 0 (tracker); fall
                        # back to the generic keyframe reborn otherwise.
                        if not self._teleport_to_motion_frame0():
                            self.env.reborn()  # pyright: ignore[reportAttributeAccessIssue]
                            self.policy.reset_alignment()
                case "[MOTION_RESET]" | "[MOTION_FADE_IN]":
                    ...
                    if self._has_default_pose_mode:
                        # Policy is already active.  In sim, snap the robot to
                        # motion frame 0 (correct height/orientation/joints) so
                        # tracking starts from a matched pose; on real hardware
                        # the teleport is a no-op and we just switch modes.
                        logger.info(
                            f"{command} — starting motion from frame 0"
                        )
                        self._teleport_to_motion_frame0()
