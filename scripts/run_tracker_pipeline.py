...
def main():
    ...
    # For RlPipelineCfg the tracker is at cfg.policy;
    # for RlLocoMimicPipelineCfg it is the first entry in cfg.mimic_policies.
    if hasattr(cfg, "policy"):
        tracker_cfg = cfg.policy
    elif hasattr(cfg, "mimic_policies") and cfg.mimic_policies:
        tracker_cfg = cfg.mimic_policies[0]
    else:
        tracker_cfg = None

    if tracker_cfg is not None:
        if args.onnx_path is not None:
            tracker_cfg.onnx_path = args.onnx_path
        if args.motion_path is not None:
            tracker_cfg.motion_path = args.motion_path
        if args.motion_index is not None:
            tracker_cfg.motion_index = args.motion_index
    ...
    if not cfg.env.is_sim:
        ...
        # RlPipeline tracker: synthetic hold until R is pressed.
        pipeline._set_default_pose_mode(True)
        logger.warning("Sim mode — holding default pose, press R to start motion")
    elif hasattr(cfg, "mimic_policies"):
        # RlLocoMimicPipeline: AMO runs immediately and keeps robot standing.
        logger.warning("Sim mode — AMO standing, press R to start motion, T to return to AMO")
